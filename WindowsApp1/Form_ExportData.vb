...
Public Class Form_ExportData
    ...
    Private Sub Form_ExportData_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        DateTimePicker_ExportStart.Value = Date.Now - TimeSpan.FromDays(30)
        DateTimePicker_ExportEnd.Value = Date.Now
    End Sub

    Private Sub Button_ExportOverTime_Click(sender As Object, e As EventArgs) Handles Button_ExportOverTime.Click
        ...
        '時間外勤務テーブルフィルタリング（指定期間内のデータ）
        Dim IsExistData As Boolean = False
        Dim drlistOverTime As DataRow() = dtOverTime.Select("workingday >= '" + DateTimePicker_ExportStart.Value + "' AND workingday <= '" + DateTimePicker_ExportEnd.Value + "'")
        If drlistOverTime.Length > 0 Then
            For Each drOverTime As DataRow In drlistOverTime
                ...
                If drOverTime("approval_division_kbn") * drOverTime("approval_department_kbn") * drOverTime("approval_bureau_kbn") * drOverTime("approval_manager_kbn") Mod 2 <> 0 Then
                    ...
                    Dim EndTime As String = Date.Parse(drOverTime("planned_end_time")).ToString("H:mm")
                    If Date.Parse(drOverTime("planned_end_time")).ToString("yyyy/MM/dd") <> "2019/01/01" Then
                        OverTime += TimeSpan.FromDays(1)
                        EndTime = "翌" + Date.Parse(drOverTime("planned_end_time")).ToString("H時mm分")
                    End If
                    ...
                    If CalculatedEndTime.Days >= 1 Then
                        ...
                        StartTime = "翌" + Date.Parse("2019/01/01 " + CalculatedEndTime.ToString).ToString("H時mm分")
                    Else
                        StartTime = Date.Parse("2019/01/01 " + CalculatedEndTime.ToString).ToString("H時mm分")
                    End If
                    '該当日～申請休憩時間を取得
                    ...
                    drCSV("勤務終了時刻") = EndTime
                    drCSV("残業開始時刻") = StartTime
                    drCSV("残業終了時刻") = EndTime
        ...
        If IsExistData Then
            'CSV出力
            Dim CSVName As String = CSVPath + "\時間外勤務テーブル_" + Date.Now.ToString("yyyyMMddHHmmss") + ".csv"
