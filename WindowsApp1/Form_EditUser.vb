...
Public Class Form_EditUser
    ...
    Private Sub Button_EditUserAddOrEdit_Click(sender As Object, e As EventArgs) Handles Button_EditUserAddOrEdit.Click
        If TextBox_EditUserName.Text = Nothing Then
            MsgBox("ユーザー名が入力されていません。", vbOKOnly, "ユーザー名エラー")
        ElseIf TextBox_EditFirstName.Text = Nothing Or TextBox_EditLastName.Text = Nothing Then
            MsgBox("氏名が入力されていません。", vbOKOnly, "氏名エラー")
        ElseIf TextBox_EditEmployeeNumber.text = Nothing Then
            MsgBox("社員番号が入力されていません。" + vbNewLine + "社員番号が発行されていない場合「99999999」と入力してください。", vbOKOnly, "社員番号エラー")
        Else
            'OracleConnectionオブジェクトを生成
            Dim con As New OracleConnection(Constr)
            'SQL文を作成
